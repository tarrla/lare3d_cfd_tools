...
Routines for reading, writing, and processing *.cfd files generated by LaRe3D_version2
Note that LaRe version >=3 use a different output format, *.SDF files.

This code is distributed via the following project:
https://github.com/tarrla/lare3d_cfd_tools

The CFD reader was heavily adapted from original code at
https://raw.githubusercontent.com/mbareford/rs-shock-heating-paper/master/scripts/python/lare3d_cfd.py

The CFD writer was subsequently adpted from the present (new) CFD reader by N.Dylan Kee (NSO, now GSFC).

Original LaRe paper by Arber+2001, JCP: 
https://ui.adsabs.harvard.edu/abs/2001JCoPh.171..151A/abstract

Most recent academic website for Lare (2024):
...
driven simulations using LaRe, Tarr+2024 ApJS 270, 30 
    ...
    See updated paper Tarr+2026, ApJ 999, 240
    DOI: 10.3847/1538-4357/ae40f4
    ADS: https://ui.adsabs.harvard.edu/abs/2026ApJ...999..240T/abstract
...
def read_lare_cfd3d(filename, verbose=False, **kwargs):
    ...
    **kwargs:
    ...
    dat : dict
        ...
        domain, `dat['rho'][0,:,:]` selects the z_min ghost cell values, and 
        `dat['bx'][1:,1:,:]` selects all in-domain values of bx (no ghost cells).
    """
    ...
    if verbose:
        ...
        print('Grid x min: ', x_min)
        print('Grid x max: ', x_max)
        print('Grid y min: ', y_min)
        print('Grid y max: ', y_max)
        print('Grid z min: ', z_min)
        print('Grid z max: ', z_max)
    ...
    for i in range(2, nblocks):
        ...
        if verbose: print('Working on block {:s}, class {:s} of total size Nx,Ny,Nz = {:4d},{:4d},{:4d}'.format(var_blk_name, var_blk_class, var_nx, var_ny,var_nz), flush=True)
        ...
        if ((var_blk_name in fields) or ('all' in fields)):
            ...
            if((not subsample_z and not subsample_y and not subsample_x) or var_blk_name=='ls'):
                ...
                var_data = np.zeros(np.prod(sub_dims)) 
...
def write_lare_cfd3d(filename, allblkdata, nvars, verbose=False, **kwargs):
    ...
    for i in range(nvars):
        ...
        elif var_blk_name.startswith('j'):
            var_blk_class = "current"
        elif var_blk_name.startswith('eta'):
            var_blk_class = "PIP"
        else:
            var_blk_class = "Fluid"
...
def get_entropy(dat):
    r'''entropy in lare units

    .. math::
       s = \log((\gamma-1)\epsilon/\rho^{\gamma-1})
    '''
    if 'gamma' in dat.keys():
        gamma = dat['gamma']
    else :
        gamma = 5./3 # default
        if(verbose): print('Gamma not found.  Using default gamma=5./3')

    return np.log((gamma-1)*dat['energy']/dat['rho']**(gamma-1))


def get_sound_speed(dat):
    '''Sound speed in lare units
    '''
    if 'gamma' in dat.keys():
        gamma = dat['gamma']
    else :
        gamma = 5./3 # default
        print('Gamma not found.  Using default gamma=5./3')

    return np.sqrt(gamma*(gamma-1)*dat['energy'])


def get_characteristic_helper_vars(dat,direction):
    ''' Calculates auxiliary variable for characteristic decomposition

    This function requires cell centered inputs.
    
    :direction: - string, one of 'x', 'y', or 'z'.  This is the third general coordinate which is the normal coordinate to a surface in the decompostion.  e.g., if direction='z' then b3=bz.
    '''
    ...
    if (direction=='z'):
        ...
        raise ValidationError('direction must be x, y, or z')
    
    tol = 1e-6
    a = get_sound_speed(dat)
    asqrtrho = a*np.sqrt(dat['rho'])
    twogamrho = 2*gamma*dat['rho']
    twogame = 2*gamma*dat['energy']
    twosqrrho = 2*np.sqrt(dat['rho'])

    alfven1 = np.abs(dat[b1])/np.sqrt(dat['rho'])
    alfven2 = np.abs(dat[b2])/np.sqrt(dat['rho'])
    alfven3 = np.abs(dat[b3])/np.sqrt(dat['rho'])
    alfvensq = alfven1**2+alfven2**2+alfven3**2
    alfvenperp =np.sqrt(alfven1**2 + alfven2**2)

    s1 = np.sign(dat[b1])
    s2 = np.sign(dat[b2])
    s3 = np.sign(dat[b3])
    
    radical = np.where(np.abs((alfvensq - alfven1**2)/a**2) < tol, 0.5*np.abs(a**2-alfven3**2),0.5*np.sqrt((a**2+alfvensq)**2 - 4*a**2*alfven3**2)  )

    cf = np.sqrt(0.5*(a**2 + alfvensq)+radical)
    cs = np.sqrt(0.5*(a**2 + alfvensq)-radical)

    beta1 = (dat[b1]+s1*1e-10)/np.sqrt(dat[b1]**2+dat[b2]**2 + 2e-20)
    beta2 = (dat[b2]+s2*1e-10)/np.sqrt(dat[b1]**2+dat[b2]**2 + 2e-20)
    beta3 = 0.0

    # ROE/BALSARA 1996 CASE III
    condition = np.squeeze(np.nonzero((alfvenperp/a < tol) * ((np.abs(a-alfven3)/a < tol)) * (a > alfven3)))
    cf[condition] = a
    cs[condition] = alfven3
    condition = np.squeeze( np.nonzero((alfvenperp/a < tol) * ((np.abs(a-alfven3)/a < tol)) * (a < alfven3)))
    # ROE/BALSARA 1996CASE IV
    cf[condition] = alfven3
    cs[condition] = a

    alphaf = np.where((np.abs(cf-cs)/a < tol),np.sin(np.arctan(alfvenperp/(alfven3 - a))/2.0)+alfvenperp/(4.0*a),np.sqrt(np.abs(a**2-cs**2)/(cf**2-cs**2)))
    alphas = np.where((np.abs(cf-cs)/a < tol),np.sqrt(1.0-alphaf**2), np.sqrt(1.0-alphaf**2) )
    ...
    dictout = {'alphas':alphas,
               ...
               'alfven3':alfven3,
               'v3':lam1}
    #'lam1':lam1,'lam2':lam2,'lam3':lam3,'lam4':lam4,'lam5':lam5,'lam6':lam6,'lam7':lam7,'lam8':lam8}
...
def get_ptot(dat):
    ...
    it is assumed to have a value of 5./3.
    '''
...
def get_pgas(dat):
    '''
    gas pressure in internal units
    P_gas = (gamma-1)rho energy
    '''
    if 'gamma' in dat.keys():
        gamma = dat['gamma']
    else :
    ...
    return (gamma-1)*dat['energy']*dat['rho']
...
def calculate_poynting(dat):
    '''Calculation involving the poynting vector

    Calculate the full poynting vector at each inteior cell vertex.  Also calculate
    the net poynting flux through each plane.

    INPUTS
    ------
    :dat: dictionary of the type returned by read_lare_cfd3().  Array shapes
          are of sizes Nx+1, Ny+1, Nz+1 (*.cfd files include 1 ghost cell for
          cell-centered type objects.  e.g., there are Nx cell center locations
          inside the simulation.)
    
    OUTPUTS
    -------
    :px: x component of poynting vector at each interior cell vertex. [Nz-1]x[Ny-1]x[Nx-1]
    :py: y component of poynting vector at each interior cell vertex. [Nz-1]x[Ny-1]x[Nx-1]
    :pz: z component of poynting vector at each interior cell vertex. [Nz-1]x[Ny-1]x[Nx-1]
    :px_rate: Net poynting flux through each constant-x plane.  [Nx-1]
    :py_rate: Net poynting flux through each constant-y plane.  [Ny-1]
    :pz_rate: Net poynting flux through each constant-z plane.  [Nz-1]
    '''
    # vertex magnetic field over all INTERIOR vertex points (no boundaries!):
    a = calculate_areas(dat)
    vareax=a['vareax']
    vareay=a['vareay']
    vareaz=a['vareaz']
    fareax=a['fareax']
    ...
    b = dat['bx'][1:,1:,1:-1]
    bxv = (b[:-1,:-1,:]*fareax[:-1,:-1,None]+
           b[1: ,:-1,:]*fareax[1:,:-1 ,None]+
           b[:-1, 1:,:]*fareax[:-1,1: ,None]+
           b[1: , 1:,:]*fareax[1:,1:  ,None])/4/vareax[:,:,None]

    b = dat['by'][1:,1:-1,1:]
    byv = (b[:-1,:,:-1]*fareay[:-1,None,:-1 ]+
           b[1: ,:,:-1]*fareay[1: ,None,:-1 ]+
           b[:-1,:, 1:]*fareay[:-1,None,1:  ]+
           b[1: ,:, 1:]*fareay[1: ,None,1:  ])/4/vareay[:,None,:]
    
    b = dat['bz'][1:-1,1:,1:]
    bzv = (b[:,:-1,:-1]*fareaz[None,:-1,:-1]+
           b[:,1: ,:-1]*fareaz[None,1:,:-1]+
           b[:,:-1, 1:]*fareaz[None,:-1,1:]+
           b[:,1: , 1:]*fareaz[None,1:,1:])/4/vareaz[None,:,:]
    

    ## calculate the poynting vector :
    # P = \vec{E}\times\vec{B} = -(\vec{v}\times\vec{B})\times\vec{B}
    # ==> P_i = B_\perp^2 v_i -= \vec{v}_\perp\cdot \vec{b}_\perp B_i:
    
    vx = dat['vx'][1:-1,1:-1,1:-1] # interior points
    vy = dat['vy'][1:-1,1:-1,1:-1] # interior points
    vz = dat['vz'][1:-1,1:-1,1:-1] # interior points
    
    # zhat direction only for now:
    px = (byv**2+bzv**2)*vx - (vy*byv + vz*bzv)*bxv 
    py = (bzv**2+bxv**2)*vy - (vz*bzv + vx*bxv)*byv 
    pz = (bxv**2+byv**2)*vz - (vx*bxv + vy*byv)*bzv

    # integrate over area to get total poynting energy flux though surface:
    px_rate = np.sum(px*vareax[:,:,None],axis=(0,1))
    py_rate = np.sum(py*vareay[:,None,:],axis=(0,2))
    pz_rate = np.sum(pz*vareaz[None,:,:],axis=(1,2))
    
    return px,py,pz, px_rate,py_rate,pz_rate
...
def calc_cell_area(xb,yb,zb):
    '''Given the rectilinear coordinates at cell boundaries, calculate cell face areas

    Note: the function calculate_areas() also gets the area on faces for each
    vertex-centered cell, which is needed to compute the Poynting flux
    ...
    OUTPUT
    ------
    :dax: x-normal area of each cell face, dimension [Nz,Ny]
    :day: y-normal area of each cell face, dimension [Nz,Nx]
    :daz: z-normal area of each cell face, dimension [Ny,Nx]
    '''
    
    dx = (np.roll(xb,-1)-xb)
    dy = (np.roll(yb,-1)-yb)
    dz = (np.roll(zb,-1)-zb)
    dax = dy[None,:-1]*dz[:-1,None]
    day = dx[None,:-1]*dz[:-1,None]
    daz = dx[None,:-1]*dy[:-1,None]
...
def calculate_currents(bxc, byc, bzc, xb, yb,zb):
    ...
    We define the Jacobian as :math:`(\nabla\boldsymbol{B})_{ij} = \partial_jB_i`.
...
def calculate_B_vertex(bxc,byc,bzc, xb,yb,zb):
    '''Calculate volume averaged magnetic field at cell vertices
    ...
    NOTES
    -----
    Not sure this is always what we want.  Sometimes using surrounding face average
    seems more appropriate.  See calculate_B_vertex_from_face().
    '''
...
def calculate_B_vertex_from_face(dat):
    '''Area average from surrounding faces.
    '''
    da = lare.calc_cell_area(dat['x'],dat['y'],dat['z'])
    
    bxv = (dat['bx'][1:-1,1:-1,1:] * da['dax'][:-1,:-1,None]+
           dat['bx'][2:  ,1:-1,1:] * da['dax'][1: ,:-1,None]+
           dat['bx'][1:-1,2:  ,1:] * da['dax'][:-1,1: ,None]+
           dat['bx'][2:  ,2:  ,1:] * da['dax'][1: ,1: ,None])/(
               da['dax'][:-1,:-1,None]+
               da['dax'][1: ,:-1,None]+
               da['dax'][:-1,1: ,None]+
               da['dax'][1: ,1: ,None])

    byv = (dat['by'][1:-1,1:,1:-1] * da['day'][:-1,None,:-1]+
           dat['by'][2:  ,1:,1:-1] * da['day'][1: ,None,:-1]+
           dat['by'][1:-1,1:,2:  ] * da['day'][:-1,None,1: ]+
           dat['by'][2:  ,1:,2:  ] * da['day'][1: ,None,1: ])/(
               da['day'][:-1,None,:-1]+
               da['day'][1: ,None,:-1]+
               da['day'][:-1,None,1: ]+
               da['day'][1: ,None,1: ])

    bzv = (dat['bz'][1:,1:-1,1:-1] * da['daz'][None,:-1,:-1]+
           dat['bz'][1:,2:  ,1:-1] * da['daz'][None,1: ,:-1]+
           dat['bz'][1:,1:-1,2:  ] * da['daz'][None,:-1,1: ]+
           dat['bz'][1:,2:  ,2:  ] * da['daz'][None,1: ,1: ])/(
               da['daz'][None,:-1,:-1]+
               da['daz'][None,1: ,:-1]+
               da['daz'][None,:-1,1: ]+
               da['daz'][None,1: ,1: ])
...
def calc_magnetic_tension_force(bxc,byc,bzc, xb,yb,zb):
    ...
    The magnetic tension :math:`\boldsymbol{f}_t` is given by

    .. math::
       \boldsymbol{f}^{(t)} = (\boldsymbol{B}\cdot\nabla)\boldsymbol{B}
...
def calc_magnetic_pressure_force(bxc,byc,bzc,xb,yb,zb):
    r'''Calculate the components of magnetic pressure.

    The magnetic tension :math:`\boldsymbol{f}_p` is given by
...
def calculate_areas(dat):
    ''' Calculate cell face areas
    
    Calculate the area of each cell face for both the simulation cells and
    the vertex-centered control volume cells.  Both are needed to properly
    compute the Poynting flux.

    Compare to calc_cell_area().
    ...
    OUTPUTS
    -------
    :area_dict: dictionary of arrays giving the area in each direction:
         :fareax/y/z: face array, combining appropriate sizes (Nx) (Ny) (Nz), 
                      e.g., fareax.shape : [Nz]x[Ny]
         :vareax/y/z: vertex arrays, combining appropriate sizes (Nx-1) (Ny-1) (Nz-1),
                      e.g., vareax.shape: [Nz-1]x[Ny-1]
    '''
    # Need to do control area averaging to get the calculation right.
    # precompute the necessary areas:
    # dx = dat['x'][1:]-dat['x'][:-1]
    # dy = dat['y'][1:]-dat['y'][:-1]
    # dz = dat['z'][1:]-dat['z'][:-1]
    
    # # face areas in each direction:
    # fareax = dz[:,None]*dy[None,:]
    # fareay = dz[:,None]*dx[None,:]
    # fareaz = dy[:,None]*dx[None,:]
    areas = calc_cell_area(dat['x'],dat['y'],dat['z'])
    fareax = areas['dax']
    fareay = areas['day']
    fareaz = areas['daz']
    
    # Vertex control area in each direction
    vareax = (fareax[:-1,:-1]+fareax[1:,:-1]+fareax[:-1,1:]+fareax[1:,1:])/4
    vareay = (fareay[:-1,:-1]+fareay[1:,:-1]+fareay[:-1,1:]+fareay[1:,1:])/4
    vareaz = (fareaz[:-1,:-1]+fareaz[1:,:-1]+fareaz[:-1,1:]+fareaz[1:,1:])/4
    
    return {'vareax':vareax,
            'vareay':vareay,
            'vareaz':vareaz,
            'fareax':fareax,
            'fareay':fareay,
